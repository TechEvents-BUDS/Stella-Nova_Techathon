...
def predict_user_input():
    ...
    fuel_consumption = float(input("Fuel Consumption (L/100 km): "))
    ...
    # Normalize the new data in the same way as the training data
    new_data_normalized = (new_data_df - np.min(X)) / (np.max(X) - np.min(X))
    ...
    predictions["Neural Network"] = nn_prediction.flatten()  # Convert 2D array to 1D array
    
    # Print predictions
    print("\nPredictions for your input:")
    for model_name, prediction in predictions.items():
        print(f"{model_name}: Predicted CO2 Emissions = {prediction[0]} g/km")
